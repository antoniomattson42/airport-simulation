...
if @airlineID_exists = 0 THEN
    leave sp_main;
    end if;
if @tail_num_exists = 1 THEN
    leave sp_main;
    end if;
...
-- INSERT INTO airplane values('Delta', 'n120jn', 10, 350, null, 'jet', null, null, 4);
end //
delimiter ;
...
if (ip_locationID IS NOT NULL) THEN
	if @locationID_exists = 0 THEN
    leave sp_main;
    end if;
end if;
INSERT INTO airport values(ip_airportID, ip_airport_name, ip_city, ip_state, 
    ip_locationID);
end //
delimiter ;
...
set maxSequence = (select max(sequence) from route_path where routeID = ip_routeID);

 if (select departure from leg where legID = ip_legID) not in (select arrival from leg where legID = 
(select legID from route_path where routeID = ip_routeID
and sequence = maxSequence)) then
	leave sp_main;
    end if;
insert into route_path values(ip_routeID, ip_legID, (maxSequence + 1));
end //
delimiter ;
...
declare neededPilots int;
declare extraTime decimal(10, 2);
declare counterInt int = 0;
set neededPilots = 1;
set extraTime = ((select distance from leg where legID = 
(select legID from route_path where sequence = 
((select (progress + 1) from flight where flightID = ip_flightID) and routeID = (select routeID from flight where flightID = ip_flightID))))
/ (select speed from airplane where tail_num = 
(select support_tail from flight where flightID = ip_flightID)));
set numPilots = (select count(*) from pilot where personID in 
(select personID from person where locationID =
(select locationID from airplane where tail_num = 
(select support_tail from flight where flightID = ip_flightID))));
 
 if 'jet' = (select plane_type from airplane where tail_num = (select support_tail from flight where flightID = ip_flightID)) then
	set neededPilots = 2;
    end if;
if 'prop' = (select plane_type from airplane where tail_num = (select support_tail from flight where flightID = ip_flightID)) then
	set neededPilots = 1;
    end if;
    
if numPilots < neededPilots then
	update flight set next_time = (addtime(next_time, '0:30:0')) where flight.flightID = ip_flightID;
    leave sp_main;
    end if;
set extraTime = extraTime * 2;
while counterInt < extraTime
begin
	update flight set next_time = (addtime(next_time, '0:30:0')) where flight.flightID = ip_flightID;
...
update person set locationID = (select locationID from airplane where tail_num = 
(select support_tail from flight where flightID = ip_flightID)) where personID in (select customer from ticket where carrier = ip_flightID) and 
locationID = 
(select locationID from airport where airportID = 
(select arrival from leg where legID = 
(select legID from route_path where sequence = 
(select progress from flight where flightID = ip_flightID) and routeID = (select routeID from flight where flightID = ip_flightID)))	);
end //
delimiter ;
...
-- select legID, departing_from, arriving_at, earliest_arrival FROM (select leg.legID, leg.departure  as 'departing_from', leg.arrival  as 'arriving_at', flight.next_time as 'earliest_arrival', flight.next_time  as 'latest_arrival', airplane.locationID  as 'airplane_list' 
-- FROM flight JOIN route_path ON flight.routeID = route_path.routeID JOIN leg ON route_path.legID = leg.legID 
-- JOIN airplane ON (flight.support_airline = airplane.airlineID AND flight.support_tail = airplane.tail_num) WHERE 
-- flight.progress = route_path.sequence AND airplane_status = 'in_flight') temp GROUP BY legID;
